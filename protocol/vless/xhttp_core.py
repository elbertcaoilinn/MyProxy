...
async def _open_tcp_from_header(first_chunk: bytes, is_trojan: bool = False):
    ...
    if _validate_target(address, port) is None:
        logger.warning(f"XHTTP SSRF-blocked dial -> {address}:{port}")
...
async def _get_or_create_session(uuid: str, mode: str, session_id: str, ip: str = "نامشخص") -> dict:
    ...
    async with XHTTP_LOCK:
        ...
        sess = {
            ...
            "dial_lock": asyncio.Lock(),  # جلوگیری از dial هم‌زمان؛ جدا از upload_lock تا
                                          # اتصال TCP (تا ۱۰ ثانیه timeout) کارهای حساسِ
                                          # بعدی روی session رو قفل نکند
        }
...
async def _open_tcp_for_session(session_id: str, uuid: str, sess: dict, first_chunk: bytes):
    """باز کردن اتصال TCP مقصد برای session — با dial_lock مجزا.

    dial (تا TCP_CONNECT_TIMEOUT ثانیه) فقط dial_lock را نگه می‌دارد، نه
    upload_lock را؛ بنابراین دو POST هم‌زمان یا downlink منتظر connect نمی‌مانند.
    اگر دو dialer روی یک session هم‌زمان بیایند، دومی همان writer اولین را می‌گیرد
    (dial_lock سریال می‌کند).
    """
    async with sess["dial_lock"]:
        # اولین dialer writer را می‌سازد؛ dialer دوم که منتظر بود نباید دوباره کانکت کند
