...
async def _open_tcp_from_trojan_header(first_chunk: bytes):
    ...
    if _validate_target(address, port) is None:
        logger.warning(f"Trojan-XHTTP SSRF-blocked dial -> {address}:{port}")
...
async def _open_tcp_for_session(session_id: str, uuid: str, sess: dict, first_chunk: bytes):
    ...
    dial (تا TROJAN_TCP_CONNECT_TIMEOUT ثانیه) فقط dial_lock را نگه می‌دارد،
    نه upload_lock را؛ دو POST هم‌زمان منتظر connect نمی‌مانند. dialer دوم که
    dial_lock را می‌گیرد، writer آماده‌ی اولی را برمی‌گرداند (نه کانکت مجدد).
    """
