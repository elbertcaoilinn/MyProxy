...
async def register_instance():
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as c:
            # ⚠️ عمداً panel_password_hash ارسال نمی‌شود — هش پسورد نباید به
            # سرویس شخص ثالث برود؛ فقط domain/version برای ثبت instance.
        ...
        logger.warning(f"central register_instance failed: {type(exc).__name__}: {exc}")
